library("cmdstanr")
library("tidyverse")
library("posterior")

options(mc.cores = parallel::detectCores())  # Use multiple cores

# Specify file path. 
# TODO upload data to github as rba.csv
FILE_PATH <- "df2.csv"
MOD_FILE_PATH <- "../cmdstanr_implementation/stanfiles/rba_uncor.stan"

# Read in the fMRI Region BOLD data.
df <- read.csv(FILE_PATH)

# Compile Stan model
mod <- cmdstan_model(MOD_FILE_PATH, compile = TRUE)

# Convert to subject and ROI to integer.
df$int_subj <- as.integer(factor(df$subject+1))
df$int_roi <- as.integer(factor(df$roi +1))

X <- model.matrix(y ~ x, data= df)

# Names correspond to the data block in the Stan program
data_list <- list(N = nrow(df), y = df$y, X = X, K = ncol(X), Kc = ncol(X) -1,
                  J = 2, N_subj = length(unique(df$subj)),
                  N_ROI=length(unique(df$roi)), subj = df$int_subj, ROI=df$int_roi)

# Fit model.
# Time taken = 
fit <- mod$sample(
  data = data_list,
  iter_warmup = 1000,                  
  iter_sampling = 1000,
  chains=4, 
  adapt_delta = .8
)

# Extract posterior draws
posterior_df <- fit$draws(format = "df")

# Alternatively, to store them in a CSV file
write.csv(posterior_df, "posterior_draws_uncor.csv", row.names = FALSE)

# Output summary to check for convergence.
# rhat is fine and ess_bulk & tail > 400.
print(fit$summary(variables = c("alpha", "b_Intercept", "tau_u") ))
